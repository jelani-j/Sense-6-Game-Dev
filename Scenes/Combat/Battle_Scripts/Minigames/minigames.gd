...
func mini_game_func(minigame_container):
	...
	var target_min = 0.45
	var target_max = 0.55
	await setup_target_zone(timing_bar, target_spot, target_min, target_max, minigame_container)
	timing_bar.add_child(target_spot)
	var result = await wait_for_input(timing_bar, target_spot, target_min, target_max, minigame_container)
...
func wait_for_input(timing_bar, target_spot, target_min, target_max, minigame_container):
	var input_recieved = false 
	var result = "missed"
	var progress = 0.0
	while input_recieved == false:
		await minigame_container.get_tree().process_frame
		progress += 1 * minigame_container.get_process_delta_time()
